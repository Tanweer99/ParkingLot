...
namespace ParkingLot.Controllers
{
    ...
    public class BookSlotController : Controller
    {
        ...
        [HttpPut]
        [Route("UpdateUserBookedSlot/{id}/{email}/{previousSlotNumber}")]
        public async Task<IActionResult> UpdateUserBookedSlot([FromBody] BookSlotDTO bookSlotDTO, string id, string email, int previousSlotNumber)
        {
            bookSlotDTO.Id = id;
            bookSlotDTO.Email = email;
            bookSlotDTO.Status = true;
            var result = await _bookSlotBLL.UpdateUserBookedSlot(id, bookSlotDTO, previousSlotNumber);
            if (result)
            {
                return Ok(result);
            }
